...
Determine if it is faster to slice data while loading from h5 or after loading.
...
def make_block(
    ...
    if not rand:
        rand = random.Random()
    slices = []
    for b_len, tot_len in zip(block_size, total_size):
        if b_len == tot_len:
...
for seed in seeds:
    ...
    for block_size in block_sizes:
        ...
        logger.info(
            "=========== Experiment {}/{} ===========".format(count, total_exps)
        )
        logger.info(
            "Params: "
        ...
        for f_idx, file_name in enumerate(filepaths):
            ...
            # Randomly pick which method to do first to avoid minimize
            # cache differences.
            if rand.random() > 0.5:
            ...
            total_seconds_per_img = (time.perf_counter() - exp_start_time) / (
                f_idx + 1
            )
            eta = datetime.timedelta(
                seconds=int(
                    total_seconds_per_img * (len(filepaths) - f_idx - 1)
                )
            )
            logger.info(
                "{}/{} ({}) h5: {:0.4f}s - numpy:{:0.4f}s - ETA: {}".format(
                    f_idx,
                    len(filepaths),
                    filenames[f_idx],
                    rt_h5[-1],
        ...
        # Write dataframe.
        df = pd.DataFrame(
            [filenames, rt_h5, rt_np],
            index=["Files", "Method 1 (H5)", "Method 2 (Numpy)"],
        ).T
