...
BASE_UNLABELED_DIR = "/dataNAS/people/arjun/data/qdess_knee_2020/unlabeled_outputs_dcm_fullanon_oct2021"

UNLABELED_SCANS = ['MTR_002', 'MTR_003', 'MTR_004', 'MTR_007', 'MTR_009',
                   'MTR_011', 'MTR_012', 'MTR_014', 'MTR_017', 'MTR_021',
                   'MTR_022', 'MTR_024', 'MTR_027', 'MTR_029', 'MTR_031',
                   'MTR_032', 'MTR_035', 'MTR_036', 'MTR_039', 'MTR_041',
                   'MTR_054', 'MTR_055', 'MTR_058', 'MTR_059', 'MTR_063',
                   'MTR_064', 'MTR_067', 'MTR_068', 'MTR_072', 'MTR_073',
                   'MTR_075', 'MTR_081', 'MTR_082', 'MTR_085', 'MTR_086',
                   'MTR_089', 'MTR_090', 'MTR_091', 'MTR_092', 'MTR_102',
                   'MTR_106', 'MTR_108', 'MTR_111', 'MTR_114', 'MTR_117',
                   'MTR_119', 'MTR_121', 'MTR_123', 'MTR_125', 'MTR_128',
                   'MTR_131', 'MTR_132', 'MTR_136', 'MTR_137', 'MTR_138',
                   'MTR_153', 'MTR_155', 'MTR_157', 'MTR_160', 'MTR_162',
                   'MTR_165', 'MTR_166', 'MTR_168', 'MTR_169', 'MTR_170',
                   'MTR_181', 'MTR_185', 'MTR_195', 'MTR_197', 'MTR_231',
                   'MTR_234', 'MTR_242', 'MTR_246', 'MTR_247', 'MTR_249']

UNUSED_SCANS = ['MTR_025', 'MTR_042', 'MTR_050', 'MTR_060', 'MTR_070',
                'MTR_076', 'MTR_078', 'MTR_088', 'MTR_134', 'MTR_141',
                'MTR_143', 'MTR_147', 'MTR_171', 'MTR_180', 'MTR_187',
                'MTR_202', 'MTR_217', 'MTR_226', 'MTR_238', 'MTR_239',
                'MTR_250']
...
QDESS_INPAINTING_CATEGORIES = [
    {
        "id": 0,
        "name": "inpainting_dummy_category",
        "abbrev": "inpainted",
        "color": [0, 0, 0]
    }
]
...
def load_2d_from_filepaths(filepaths: list,
                           source_path: str,
...
def load_2d_from_dir(scan_root: str,
                     dataset_name: str = None):
    files = sorted(os.listdir(scan_root))
    filepaths = [os.path.join(scan_root, f) for f in files if f.endswith(".im")]

    return load_2d_from_filepaths(filepaths,
                                  scan_root,
                                  dataset_name=dataset_name)


def load_2d_from_json(json_path: str,
                      dataset_name: str = None):
...
def load_qdess(json_file, dataset_name=None):
    ...
    dataset_dicts = []
    for json_path, data_name in zip(json_files, dataset_names):
...
def register_qdess_dataset(scan_root: str,
                           dataset_name: str):
    """
    ...
    DatasetCatalog.register(dataset_name,
                            lambda: load_func(scan_root, dataset_name))
    ...
    MetadataCatalog.get(dataset_name).set(
        ...
        category_abbreviations={
            "segmentation": [
                x["abbrev"] for x in QDESS_SEGMENTATION_CATEGORIES
            ],
            "inpainting": [x["abbrev"] for x in QDESS_INPAINTING_CATEGORIES]
        },
        categories={
            "segmentation": [
                x["name"] for x in QDESS_SEGMENTATION_CATEGORIES
            ],
            "inpainting": [
                x["name"] for x in QDESS_INPAINTING_CATEGORIES
            ]
        },
        category_colors={
            "segmentation": [
                x["color"] for x in QDESS_SEGMENTATION_CATEGORIES
            ],
            "inpainting": [
                x["color"] for x in QDESS_INPAINTING_CATEGORIES
            ]
        },
...
def register_all_qdess_datasets():
    ...
    for dataset_name, scan_root in _DATA_CATALOG.items():
        register_qdess_dataset(scan_root=scan_root,
                               dataset_name=dataset_name)
