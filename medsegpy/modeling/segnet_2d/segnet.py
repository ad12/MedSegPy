...
class SegNet(KModel):
        ...
        for i in reversed(range(depth)):
            ...
            if conv_act_bn:
                curr_layer = self._decoder_block_conv_act_bn(
                    ...
                    num_filters=
                    num_filters[i],
                    num_filters_next=1 if i == 0 else
                    num_filters[i - 1],
                    kernel=kernel,
                    pool_size=eff_pool_size,
                )
    ...
    def _encoder_block(self, x, level, num_conv_layers = 2, num_filters = 64,
                       kernel = 3, pool_size = (2, 2), single_bn = False,
                       add_pool: bool=True):
        seed = self._seed
        ...
        for i in range(num_conv_layers):
            ...
            if not single_bn:
                conv = BatchNormalization(
                    name='enc_%d_bn_%d' % (level, i + 1)
                )(conv)
            conv = Activation(
                "relu",
                name='enc_%d_relu_%d' % (level, i + 1)
            )(conv)
            curr_layer = conv

        if single_bn:
            curr_layer = BatchNormalization(
                name='enc_%d_bn' % level
            )(curr_layer)
    ...
    def _decoder_block(self, x_pool, x_mask, level, num_conv_layers = 2,
                       num_filters = 64, num_filters_next = 32, kernel = 3,
    ...
    def _encoder_block_conv_act_bn(
        ...
        if add_pool:
            l_pool, l_mask = MaxPoolingWithArgmax2D(pool_size=pool_size,
                                                    strides=pool_size,
                                                    name='enc_%d_pool' % level)(
                curr_layer)
        else:
    ...
    def _decoder_block_conv_act_bn(self, x_pool, x_mask, level,
                                   num_conv_layers = 2, num_filters = 64,
                                   num_filters_next = 32,
                                   kernel = 3,
                                   pool_size = (2, 2)):
        seed = self._seed
