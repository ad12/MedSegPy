...
def train_model(config, optimizer=None, model=None, class_weights=None):
    ...
    # model callbacks
    cp_cb = ModelCheckpoint(os.path.join(output_dir, cp_save_tag + '_weights.{epoch:03d}-{val_loss:.4f}.h5'),
    ...
    if k_fold_cross_validation:
        ...
        for tr_f, val_f, test_f, tr_bins, val_bins, test_bins in cv_wrapper.run():
            c.init_cross_validation(train_files=tr_f,
                                    ...
                                    cv_k=cv_k,
                                    cv_file=cv_file,
                                    output_dir=os.path.join(base_save_path, 'cv-exp-%03d' % cv_exp_id))
            cv_exp_id += 1
