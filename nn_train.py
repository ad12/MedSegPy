...
class NNTrain(CommandLineInterface):
    ...
    __ARG_KEY_SAVE_MODEL = 'save_model'

    @property
    def config(self):
        self.verify_args()
        return self.args[self.__ARG_KEY_CONFIG__]

    @property
    def save_best_weights(self):
        self.verify_args()
        return not self.args[self.__ARG_KEY_SAVE_ALL_WEIGHTS__]

    @property
    def save_model(self):
        return self.args[self.__ARG_KEY_SAVE_MODEL]

    @property
    def frozen_layers(self):
        frozen_layers = self.get_arg(self.__ARG_KEY_FREEZE_LAYERS__)
    ...
    def init_parser(self):
        arg_subparser = self.base_parser.add_subparsers(help='supported configs for different architectures',
                                                        dest=self.__ARG_KEY_CONFIG__)
        subparsers = MCONFIG.init_cmd_line_parser(arg_subparser)

        for s_parser in subparsers:
            self.__add_gpu__argument__(s_parser)
            s_parser.add_argument('-k', '--%s' % self.__ARG_KEY_K_FOLD_CROSS_VALIDATION__,
                                  metavar='K', default=None, nargs='?',
                                  dest=self.__ARG_KEY_K_FOLD_CROSS_VALIDATION__,
                                  help='Use k-fold cross-validation for training. '
                                       'Argument is k (int) or filepath (str).')
            s_parser.add_argument('--%s' % self.__ARG_KEY_HO_TEST__,
                                  metavar='T', type=int, default=1, nargs='?',
                                  dest=self.__ARG_KEY_HO_TEST__,
                                  help='Number of hold-out test bins.')
            s_parser.add_argument('--%s' % self.__ARG_KEY_HO_VALID__,
                                  metavar='V', type=int, default=1, nargs='?',
                                  dest=self.__ARG_KEY_HO_VALID__,
                                  help='Number of hold-out validation bins.')
            s_parser.add_argument('--%s' % self.__ARG_KEY_CLASS_WEIGHTS__,
                                  type=tuple, nargs='?', default=None,
                                  dest=self.__ARG_KEY_CLASS_WEIGHTS__,
                                  help='Weight classes in order.')
            s_parser.add_argument('--%s' % self.__ARG_KEY_EXPERIMENT__,
            ...
            s_parser.add_argument('--%s' % self.__ARG_KEY_FREEZE_LAYERS__,
                                  type=str, default=None, nargs='?',
                                  dest=self.__ARG_KEY_FREEZE_LAYERS__,
                                  help='Range of layers to freeze. eg. `(0,100)`, `(5, 45)`, `(5,)`')
            s_parser.add_argument('--%s' % self.__ARG_KEY_SAVE_ALL_WEIGHTS__,
                                  default=False, action='store_const', const=True,
                                  dest=self.__ARG_KEY_SAVE_ALL_WEIGHTS__,
                                  help="Store weights at each epoch. Default: False")
            s_parser.add_argument('--%s' % self.__ARG_KEY_SAVE_MODEL,
                                  default=False, action='store_const', const=True,
                                  dest=self.__ARG_KEY_SAVE_MODEL,
    ...
    def run(self):
        ...
        c = MCONFIG.get_config(config_cp_save_tag=self.args[self.__ARG_KEY_CONFIG__],
    ...
    def __train_cross_validation(self, c, config_dict):
        k_fold_cross_validation = self.get_arg(self.__ARG_KEY_K_FOLD_CROSS_VALIDATION__)
        if k_fold_cross_validation.isdigit():
            k_fold_cross_validation = int(k_fold_cross_validation)

        ho_valid = self.get_arg(self.__ARG_KEY_HO_VALID__)
        ho_test = self.get_arg(self.__ARG_KEY_HO_TEST__)
    ...
    def __train_fine_tune(self, config, vals_dict=None):
        dirpath = self.get_arg(self.__ARG_KEY_FINE_TUNE_PATH__)
