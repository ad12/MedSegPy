...
def interp_slice(y_true, y_pred, orientation="M"):
    ...
    xp = (
        (np.asarray(list(range(start, stop + 1))) - start)
        / (stop - start)
        * 100.0
    )
...
def test_model(
    ...
    # TODO: Remove `metrics=...`.
    metrics = (
        [SegMetric.DSC, SegMetric.VOE, SegMetric.ASSD, SegMetric.CV]
        if not metrics
        else metrics
    )
    ...
    for x_test, y_test, recon, fname, seg_time in test_gen.img_generator_test(
        model
    ):
        ...
        if not voxel_spacing:
            ...
            if not isinstance(voxel_spacing, tuple) or len(voxel_spacing) != 3:
                raise ValueError(
                    "voxel_spacing %s not supported." % str(voxel_spacing)
                )
        ...
        logger_info_str = "Scan #%03d (name = %s, %d slices, %0.2fs) = %s" % (
            img_cnt,
            fname,
            num_slices,
            seg_time,
    ...
    sio.savemat(
        os.path.join(test_result_path, "total_interp_data.mat"),
        {"xs": x_interp, "ys": y_interp, "xt": x_total, "yt": y_total},
    )
...
def find_best_test_dir(base_folder):
    ...
    for subdir in subdirs:
        ...
        assert not ((results_files is None) or (len(results_files) == 0)), (
            "Checking results file failed - %s" % subdir
        )
...
def test_dir(
    ...
    if not config:
        config = MCONFIG.get_config(
            MCONFIG.get_model_name(config_filepath), create_dirs=False
        )
...
ARCHITECTURE_PATHS_PREFIX = (
    "/bmrNAS/people/arjun/msk_seg_networks/architecture_limit/%s"
)
DATA_LIMIT_PATHS_PREFIX = os.path.join(
    "/bmrNAS/people/arjun/msk_seg_networks/data_limit", "%03d", "%s"
)
AUGMENTATION_PATH_PREFIX = os.path.join(
    "/bmrNAS/people/arjun/msk_seg_networks/augment_limited", "%s"
)
LOSS_PATH_PREFIX = os.path.join(
    "/bmrNAS/people/arjun/msk_seg_networks/loss_limit", "%s"
)
VOLUME_PATH_PREFIX = os.path.join(
    "/bmrNAS/people/arjun/msk_seg_networks/volume_limited", "%s"
)
BEST_NETWORK_PATHS_PREFIX = (
    "/bmrNAS/people/arjun/msk_seg_networks/best_network/%s"
)
EXP_KEY = "exp"
...
def get_config(name):
    configs = [
        DeeplabV3Config(create_dirs=False),
        UNetConfig(create_dirs=False),
        SegnetConfig(create_dirs=False),
        UNet2_5DConfig(create_dirs=False),
        ResidualUNet(create_dirs=False),
    ]
...
def init_deeplab_parser(parser):
    parser.add_argument(
        "-%s" % OS_KEY, nargs="?", default=None, choices=[8, 16]
    )
    parser.add_argument(
        "-%s" % DIL_RATES_KEY, nargs="?", default=None, type=tuple
    )
...
def add_base_architecture_parser(
    architecture_parser,
    supported_architectures=SUPPORTED_ARCHITECTURES,
    add_vals=[],
):
    for architecture in supported_architectures:
        parser = architecture_parser.add_parser(
            architecture, help="use %s" % architecture
        )
        parser.add_argument(
            "-%s" % BATCH_TEST_KEY,
            action="store_const",
...
def init_architecture_parser(input_subparser):
    subparser = input_subparser.add_parser(
        "arch", help="test architecture experiment"
    )
    architecture_parser = subparser.add_subparsers(
        help="architecture to use", dest=ARCHITECTURE_KEY
    )
...
def init_data_limit_parser(input_subparser):
    subparser = input_subparser.add_parser(
        "data", help="test data limitation experiment"
    )
    architecture_parser = subparser.add_subparsers(
        help="architecture to use", dest=ARCHITECTURE_KEY
    )
...
def handle_architecture_exp(vargin):
    ...
    if date is None:
        raise ValueError(
            "Must specify either 'date' or '%s'" % (BATCH_TEST_KEY)
        )

    fullpath = os.path.join(architecture_folder_path, date)
    if not os.path.isdir(fullpath):
        raise NotADirectoryError(
            "%s does not exist. Make sure date is correct" % fullpath
        )
...
def handle_data_limit_exp(vargin):
    ...
    for count in [5, 15, 30, 60]:
        architecture_folder_path = DATA_LIMIT_PATHS_PREFIX % (
            count,
            config_name,
        )
...
def init_augment_limit_parser(input_subparser):
    subparser = input_subparser.add_parser(
        "aug", help="test augmentation experiment"
    )
    architecture_parser = subparser.add_subparsers(
        help="architecture to use", dest=ARCHITECTURE_KEY
    )
...
def init_loss_limit_parser(input_subparser):
    subparser = input_subparser.add_parser(
        "loss", help="test loss experiment (DSC vs weighted CE)"
    )
    architecture_parser = subparser.add_subparsers(
        help="architecture to use", dest=ARCHITECTURE_KEY
    )
...
def handle_loss_limit_exp(vargin):
    ...
    if do_batch_test:
        batch_test(
            loss_folder_path, config_name, [vals_dict], overwrite=overwrite_data
        )
        return
...
def init_volume_limit_parser(input_subparser):
    subparser = input_subparser.add_parser(
        "vol", help="test volume experiment (2.5D/3D)"
    )
    architecture_parser = subparser.add_subparsers(
        help="architecture to use", dest=ARCHITECTURE_KEY
    )
...
def handle_fcn_test_parser(vargin):
    ...
    for test_folder, img_size in test_folders:
        ...
        if do_batch_test:
            batch_test(
                folder_path, config_name, [vals_dict], overwrite=overwrite_data
            )
            continue
...
def init_best_network_test_parser(input_subparser):
    subparser = input_subparser.add_parser(
        "best", help="test best trained experiment"
    )
    architecture_parser = subparser.add_subparsers(
        help="architecture to use", dest=ARCHITECTURE_KEY
    )
