...
UNET_3D_TEST_PATH = (
    "/bmrNAS/people/arjun/msk_seg_networks/volume_limited/unet_3d_asc/test"
)
...
TEST_SET_METADATA_PIK = (
    "/bmrNAS/people/arjun/msk_seg_networks/oai_data_test/oai_test_data.dat"
)
...
def load_pid_data(dirpath=UNET_3D_TEST_PATH):
    ...
    for file in files:
        ...
        im1 = io_utils.load_h5(os.path.join(dirpath, "%s_1.im" % scan_id))[
            "data"
        ][:]
        im2 = io_utils.load_h5(os.path.join(dirpath, "%s_2.im" % scan_id))[
            "data"
        ][:]

        seg1 = io_utils.load_h5(os.path.join(dirpath, "%s_1.seg" % scan_id))[
            "data"
        ][:]
        seg2 = io_utils.load_h5(os.path.join(dirpath, "%s_2.seg" % scan_id))[
            "data"
        ][:]
        seg_ind = 0
        ...
        pred1 = io_utils.load_h5(os.path.join(dirpath, "%s_1.pred" % scan_id))[
            "pred"
        ][:]
        pred2 = io_utils.load_h5(os.path.join(dirpath, "%s_2.pred" % scan_id))[
            "pred"
        ][:]
...
def test_model():
    ...
    for fname in scans_keys:
        ...
        logger.info_str = (
            "DSC, VOE, CV for image #%d (name = %s, %d slices) = %0.3f, %0.3f, %0.3f"
        ...
        xs, ys, xt, yt = oai_test.interp_slice(
            y_test, labels, orientation=slice_dir
        )
        x_interp.append(xs)
        y_interp.append(ys)
        x_total.append(xt)
        ...
        if save_file == 1:
            ...
            # Save mask overlap
            ovlps = im_utils.write_ovlp_masks(
                os.path.join(test_result_path, "ovlp", fname), y_test, labels
            )
            im_utils.write_mask(
                os.path.join(test_result_path, "gt", fname), y_test
            )
            im_utils.write_prob_map(
                os.path.join(test_result_path, "prob_map", fname), labels
            )
            im_utils.write_im_overlay(
                os.path.join(test_result_path, "im_ovlp", fname), x_write, ovlps
            )
    ...
    stats_string = oai_test.get_stats_string(
        dice_losses, voes, cv_values, skipped_count, end - start
    )
    ...
    metrics = {"dsc": dice_losses, "voe": voes, "cvs": cv_values}
