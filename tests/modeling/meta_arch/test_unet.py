...
from keras.layers import Concatenate, Conv3D, Activation, Conv3DTranspose
from keras.utils import plot_model

from medsegpy.config import UNetConfig, UNet2_5DConfig, UNet3DConfig
    ...
    def _compare_layers(cls, m1, m2):
        for l1, l2 in zip(m1.layers, m2.layers):
            ...
            if isinstance(l2, Concatenate):
                channel_axis = -1 if K.image_data_format() == "channels_last" else 1  # noqa
...
class TestUNet3D(unittest.TestCase):
    ...
    def test_same_unet_3d(self):
        ...
        while True:
            ...
            assert not reached_l1 and not reached_l2, (
            ...
            if isinstance(l2, Concatenate):
                ...
                assert isinstance(l1_activation, Activation), (
                ...
                assert l1_activation == l2_activation, (
                    "l1_idx: {} - {}\nl2_cfg: {} - {}".format(
